                ...
                <div class="col-span-full ">
                    <div class="flex justify-start items-center ">
                        <div class="flex flex-row items-center justify-center gap-2">
                            ...
                            <div class=" self-stretch w-1 h-auto bg-yellow-500 rounded-xl"></div>
                            <div
            ...
            @foreach ($dataMenu as $item)
                @if (empty($item->children))
                    <a wire:navigate href="{{ $item->route ?? null }}"
                        class="{{ $item->isActive ? 'text-yellow-700' : null }} relative text-normal font-medium rounded-xl transition duration-200 ease-in-out hover:text-yellow-700 group">
                        {{ $item->title ?? null }}
                        <span
                            class="absolute left-1/2 -bottom-1 w-10 h-1 bg-yellow-500 rounded-full -translate-x-1/2 transform scale-x-0 group-hover:scale-x-100 transition-transform duration-300 origin-center"></span>
                    </a>
                @else
                    <div class="relative">
                        <button popovertarget="desktop-menu-product"
                            class="group flex items-center gap-x-1 text-normal font-medium text-slate-900 hover:text-yellow-700 transition duration-200 ease-in-out rounded-xl">
                            <span class="relative">
                                {{ $item->title ?? null }}
                                <span
                        ...
                        <el-popover id="desktop-menu-product" anchor="bottom" popover
                            ...
                            @foreach ($item->children as $item)
                                <div
                                    class="group relative flex items-center gap-4 px-4 py-2 text-sm hover:bg-gray-100 ">
                                    <div class="flex-auto">
                                        <a wire:navigate href="{{ $item->route ?? null }}"
                                            class="block font-semibold text-base text-slate-600 hover:text-yellow-600">
                                            {{ $item->title ?? null }}
